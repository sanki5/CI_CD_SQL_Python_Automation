...
/****** Object:  StoredProcedure [dbo].[spdHotelOrphans]    Script Date: 2/6/2020 9:04:18 PM ******/
DROP PROCEDURE IF EXISTS [dbo].[spdHotelOrphans]
GO
...
CREATE PROCEDURE [dbo].[spdHotelOrphans]  
...
AS
/*************************************************************************************  
* AUTHOR: Blaise Ziemian  
* CREATED: 7/01/2016  
* PURPOSE: Delete orphaned dimHotelHMF records from Conforming Dimensions, PreTrip, and PostTrip
*************************************************************************************/  

/*
2017-02-07	B Ziemian		Added Rowlocks to delete statements.
							Added a two-second wait at the end of the deletes to allow other programs to access the tables.

Version		Updated By			Updated Date			Description
--------	-----------			-------------			------------------------
V1			Ankur Adarsh		12-26-2017				Added SET DEADLOCK_PRIORITY LOW as per DSD-1110. 
V2  		Ankur Adarsh		05-10-2018				Added the logic to avoid the cancelled Hotel record to
														get deleted from hotel tables as per BCDETL-xxx. 
...
SET @TaskNameLoop  = 'HotelOrphan Delete Loop'
SET @TaskNameConfDim  = 'Delete Conforming Dimensions dimHotelHMF'
SET @TaskNamePreTrip  = 'Delete PreTrip dimHotelHMF'
SET @TaskNamePostTrip = 'Delete PostTrip dimHotelHMF'
SET @TaskNameODSLkupHotel = 'Delete ODSLkupHotel'
SET @TaskNameODSLkupHotelBCDMaster = 'Delete ODSLkupHotelBCDMaster'
...
--V3 End
	...
	--ODSMasterTables_Stage.dbo.HotelOrphanPurge P (NOLOCK)
	ON A.HotelID = P.HotelID
	--WHERE P.RowID BETWEEN @MinRowID AND @MaxRowID
...
INTO   dbo.#ConflictCancelledHotel
...
JOIN dbo.#ConflictCancelledHotel ct
ON da.HotelID = ct.HotelID
WHERE NOT EXISTS (SELECT 1 FROM [ODSMasterTables].[dbo].[ODSLkupHotel] oh (NOLOCK) WHERE oh.HotelID = da.HotelID);
...
ON A.HotelID = B.HotelID 
WHERE NOT EXISTS (SELECt 1 FROM dbo.#ConflictCancelledHotel ct WHERE ct.HotelKey = B.HotelKey)--V2
